...
g.test_3010_collate_after_limit = function(cg)
    cg.server:exec(function()
        -- All of these tests should throw the "COLLATE cannot be used in LIMIT
        -- and OFFSET" error.
...
--
-- gh-13235: a non-existent collation is now resolved eagerly, so the
-- error is raised even in positions where the collation is ignored.
--
g.test_13235_collation_resolved_eagerly = function(cg)
    cg.server:exec(function()
        local exp_err = "Collation 'not_exist' does not exist"
        -- The COLLATE result is never used, but the error is raised.
        local _, err = box.execute("SELECT 'asd' COLLATE not_exist;")
        t.assert_equals(err.message, exp_err)
        _, err = box.execute("SELECT 'asd' COLLATE not_exist AS x;")
        t.assert_equals(err.message, exp_err)
    end)
end

--
-- gh-13235: eager resolution reports a non-existent collation before the
-- "COLLATE cannot be used in LIMIT and OFFSET" error.
--
g.test_13235_collation_error_precedes_limit = function(cg)
    cg.server:exec(function()
        local exp_err = "Collation 'not_exist' does not exist"
        local _, err = box.execute("SELECT 1 LIMIT 1 COLLATE not_exist;")
        t.assert_equals(err.message, exp_err)
        _, err = box.execute("SELECT 1 LIMIT 1 OFFSET '1' COLLATE not_exist;")
        t.assert_equals(err.message, exp_err)
    end)
end
