            ...
            peer = schema.record({
                ...
                uri = schema.scalar({
                    ...
                    validate = function(data, w)
                        ...
                        if uri.login ~= nil then
                            w.error('Login for iproto.advertise.peer must ' ..
                                    'be set via iproto.advertise.peer.login')
                        end
                        assert(uri.password == nil)
                        if uri.params ~= nil then
                            w.error('Parameters for iproto.advertise.peer ' ..
                                    'must be set via ' ..
                                    'iproto.advertise.peer.params')
                        end
                        local ok, err = uri_is_suitable_to_connect(uri)
                        if not ok then
                            w.error(err)
                ...
                validate = function(data, w)
                    ...
                    if data.password ~= nil and data.login == nil then
                        w.error('Password cannot be set witout setting login')
                    end
                    -- If a params is set, an uri must also be specified.
                    if data.params ~= nil and data.uri == nil then
                        w.error('Params cannot be set witout setting uri')
                    end
                end,
            }),
            sharding = schema.record({
                ...
                uri = schema.scalar({
                    ...
                    validate = function(data, w)
                        ...
                        if uri.login ~= nil then
                            w.error('Login for iproto.advertise.sharding ' ..
                                    'must be set via ' ..
                                    'iproto.advertise.sharding.login')
                        end
                        assert(uri.password == nil)
                        if uri.params ~= nil then
                            w.error('Parameters for ' ..
