...
select_from_ast(struct Parse *parser, struct ast_select *select)
{
	if (select == NULL)
		return NULL;
	/*
	 * Convert the compound parts from left to right, so that anonymous
	 * bind variables ("?") are numbered in the order they appear in the
	 * query. The rightmost part is stored in @select and is converted
	 * last; it becomes the head of the resulting pPrior chain.
	 */
