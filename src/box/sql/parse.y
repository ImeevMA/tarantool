...
column_name_and_type ::= nm(A) typedef(Y). {
  sql_ast_save_table_column(pParse, &A, Y.type);
}

create_column_end ::= autoinc(I). {
  if (I == 1)
    sql_ast_save_column_autoincrement(pParse);
}
columnlist ::= tcons.
...
ccons ::= DEFAULT term(X). {
  sql_ast_save_column_default(pParse, &X);
}
...
ccons ::= DEFAULT MINUS(A) term(X).      {
  ...
  v.zStart = A.z;
  v.zEnd = X.zEnd;
  sql_ast_save_column_default(pParse, &v);
}
...
//
ccons ::= NULL onconf(R).        {
  sql_ast_save_column_null_action(pParse, ON_CONFLICT_ACTION_NONE, R);
}
ccons ::= NOT NULL onconf(R). {
  sql_ast_save_column_null_action(pParse, R, R);
}
ccons ::= cconsname(N) PRIMARY KEY sortorder(Z). {
  sql_ast_save_column_primary_key(pParse, &N, Z);
}
ccons ::= cconsname(N) UNIQUE. {
  sql_ast_save_column_unique(pParse, &N);
}

ccons ::= cconsname(N) CHECK LP expr(X) RP. {
  sql_ast_save_column_check(pParse, &N, &X);
}

ccons ::= cconsname(N) REFERENCES nm(T) eidlist_opt(TA). {
  sql_ast_save_column_foreign_key(pParse, &N, &T, TA);
}
ccons ::= COLLATE id(C). {
  sql_ast_save_column_collate(pParse, &C);
}
...
tcons ::= cconsname(N) PRIMARY KEY LP col_list_with_autoinc(X) RP. {
  sql_ast_save_table_primary_key(pParse, &N, X);
}
tcons ::= cconsname(N) UNIQUE LP sortlist(X) RP. {
  sql_ast_save_table_unique(pParse, &N, X);
}
tcons ::= cconsname(N) CHECK LP expr(X) RP. {
  sql_ast_save_table_check(pParse, &N, &X);
}
tcons ::= cconsname(N) FOREIGN KEY LP eidlist(FA) RP
          REFERENCES nm(T) eidlist_opt(TA). {
  sql_ast_save_table_foreign_key(pParse, &N, FA, &T, TA);
}
...
cmd ::= createkw(X) VIEW ifnotexists(E) nm(Y) eidlist_opt(C)
  ...
  if (!pParse->parse_only) {
    sql_ast_init_create_view(pParse, &Y, &X, C, S, E);
  } else {
...
col_list_with_autoinc(A) ::= col_list_with_autoinc(A) COMMA expr(Y)
                             autoinc(I). {
  if (I == 1)
    sql_ast_save_table_autoincrement(pParse, sqlExprDup(Y.pExpr, 0));
  A = sql_expr_list_append(A, Y.pExpr);
}
...
cmd ::= createkw is_unique(U) INDEX ifnotexists(NE) nm(X)
  ...
  sql_ast_init_create_index(pParse, &Y, &X, Z, U, NE);
}
...
alter_column_def ::= alter_add_column(N) typedef(Y). {
  sql_ast_init_add_column(pParse, N.table_name, &N.name, Y.type);
}

cmd ::= alter_add_constraint(N) FOREIGN KEY LP eidlist(FA) RP REFERENCES
        nm(T) eidlist_opt(TA). {
  sql_ast_init_add_foreign_key(pParse, N.table_name, &N.name, FA, &T, TA);
}

cmd ::= alter_add_constraint(N) CHECK LP expr(X) RP. {
  sql_ast_init_add_check(pParse, N.table_name, &N.name, &X);
}

cmd ::= alter_add_constraint(N) UNIQUE LP sortlist(X) RP. {
  sql_ast_init_add_unique(pParse, N.table_name, &N.name, X);
}

cmd ::= alter_add_constraint(N) PRIMARY KEY LP sortlist(X) RP. {
  sql_ast_init_add_primary_key(pParse, N.table_name, &N.name, X);
}
