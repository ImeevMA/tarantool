...
%fallback ID
  ...
  RENAME CTIME_KW IF ENABLE DISABLE UUID SHOW INCLUDING ERRORS
  .
%wildcard WILDCARD.
...
cmd ::= SHOW CREATE TABLE nm(X). {
  sql_emit_show_create_table_throw(pParse, &X);
}
cmd ::= SHOW CREATE TABLE nm(X) INCLUDING ERRORS. {
  sql_emit_show_create_table_include(pParse, &X);
}
cmd ::= SHOW CREATE TABLE. {
  sql_emit_show_create_tables_ignore(pParse);
}
cmd ::= SHOW CREATE TABLE INCLUDING ERRORS. {
